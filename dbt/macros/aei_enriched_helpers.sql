{% macro aei_enriched_filtered_geo_ids(source_relation, geography, min_usage) -%}
select distinct
    geo_id
...
{% macro aei_enriched_filtered_geography_ctes(source_relation='scaffold_rows') -%}
filtered_countries as (

    {{ aei_enriched_filtered_geo_ids(source_relation, 'country', 200) }}

),

filtered_us_states as (

    {{ aei_enriched_filtered_geo_ids(source_relation, 'country-state', 100) }}

)
{%- endmacro %}

{% macro aei_enriched_threshold_eligible_geography_condition(
...
{% macro aei_enriched_is_classified_cluster(cluster_column='cluster_name') -%}
